...
class Q(object):
    ...
    def __init__(
        self, values: List = None, q_type: object = "Q", representation: str = ""
    ):
        ...
        if values is None:
            ...
            raise ValueError(
                f"The program accepts lists/arrays of 4 or 8 dimensions, not {len(values)}"
            )
    ...
    def print_state(
        self: Q, label: str = "", spacer: bool = True, quiet: bool = True
    ) -> None:
        """
    ...
    def check_representations(self: Q, q_2: Q) -> bool:
        ...
        if self.representation == q_2.representation:
            ...
            raise Exception(
                f"Oops, 2 have different representations: {self.representation} {q_2.representation}"
            )
...
class Qs(object):
    ...
    def set_qs_type(
        self: Qs, qs_type: str = "", rows: int = 0, columns: int = 0, copy: bool = True
    ) -> Qs:
        """
    ...
    def print_state(
        self: Qs, label: str = "", spacer: bool = True, quiet: bool = True
    ) -> None:
        """
    ...
    def op_q(
        self: Qs, q: Qs, first: bool = True, kind: str = "", reverse: bool = False
    ) -> Qs:
        """
...
def qq_to_qs_function(func, q_1, q_2):
    ...
    return Qs(
        [func(q, r) for q, r in zip(q_1.qs, q_2.qs)],
        qs_type=q_1.qs_type,
        rows=q_1.rows,
        columns=q_1.columns,
    )
...
def qqq_to_qs_function(func, q_1, q_2, q_3):
    ...
    return Qs(
        [func(q, r, s) for q, r, s in zip(q_1.qs, q_2.qs, q_3.qs)],
        qs_type=q_1.qs_type,
        rows=q_1.rows,
        columns=q_1.columns,
    )
...
def vector_q(q_1: Q) -> Q:
    ...
    v = Q(
        [0, q_1.x, q_1.y, q_1.z], q_type=end_q_type, representation=q_1.representation
    )
    return v
...
def norm_squared_of_vectors(q_1: Qs) -> Qs:
    ...
    return products(
        vector_qs(conjs(q_1.set_qs_type("bra"))), vector_qs(q_1.set_qs_type("ket"))
    )
...
def normalizes(q_1: Qs, n: float = 1.0) -> Qs:
    ...
    return Qs(
        new_states_normalized, qs_type=q_1.qs_type, rows=q_1.rows, columns=q_1.columns
    )
...
def determinant(self: Qs) -> Qs:
    ...
    if self.dim == 1:
        ...
        raise ValueError(
            "Oops, don't know how to calculate the determinant of this one."
        )

    return Qs([q_det], qs_type=self.qs_type, rows=1, columns=1)
...
def products(q_1: Qs, q_2: Qs, kind: str = "", reverse: bool = False) -> Qs:
    ...
    result = [
        [q0(q_type="") for _i in range(outer_column_max)] for _j in range(outer_row_max)
    ]
    ...
    for outer_row in range(outer_row_max):
        for outer_column in range(outer_column_max):
            for shared_inner in range(shared_inner_max):
                ...
                result[outer_row][outer_column] = add(
                    result[outer_row][outer_column],
                    product(
                        qs_left.qs[left_index],
                        qs_right.qs[right_index],
                        kind=kind,
                        reverse=reverse,
                    ),
                )
    ...
    if q_1.qs_type in ["op", "operator"]:
        ...
        if additive:
            ...
            if q_1.dim == 1:
                ...
                q_inv = Qs(
                    new_qs, qs_type=q_1.qs_type, rows=q_1.rows, columns=q_1.columns
                )
                ...
                q0 = product(
                    dif(q_1.qs[4], product(q_1.qs[8]), product(q_1.qs[5], q_1.qs[7])),
                    detinv,
                )

                q_2 = product(
                    dif(q_1.qs[7], product(q_1.qs[2]), product(q_1.qs[8], q_1.qs[1])),
                    detinv,
                )

                q2 = product(
                    dif(q_1.qs[1], product(q_1.qs[5]), product(q_1.qs[2], q_1.qs[4])),
                    detinv,
                )

                q3 = product(
                    dif(q_1.qs[6], product(q_1.qs[5]), product(q_1.qs[8], q_1.qs[3])),
                    detinv,
                )

                q4 = product(
                    dif(q_1.qs[0], product(q_1.qs[8]), product(q_1.qs[2], q_1.qs[6])),
                    detinv,
                )

                q5 = product(
                    dif(q_1.qs[3], product(q_1.qs[2]), product(q_1.qs[5], q_1.qs[0])),
                    detinv,
                )

                q6 = product(
                    dif(q_1.qs[3], product(q_1.qs[7]), product(q_1.qs[4], q_1.qs[6])),
                    detinv,
                )

                q7 = product(
                    dif(q_1.qs[6], product(q_1.qs[1]), product(q_1.qs[7], q_1.qs[0])),
                    detinv,
                )

                q8 = product(
                    dif(q_1.qs[0], product(q_1.qs[4]), product(q_1.qs[1], q_1.qs[3])),
                    detinv,
                )
        ...
        q_inv = Qs(new_states, qs_type=q_1.qs_type, rows=q_1.rows, columns=q_1.columns)
...
def rotation_angle(
    ...
    q_1__q_2 = normalize(
        product(q_1_shifted_vector, conj(q_2_shifted_vector)), n=tangent_space_norm
    )
...
def identity(
    ...
    if additive:
        ...
        if len(non_zeroes) != dim:
            raise ValueError(
                f"Oops, len(non_zeroes)={len(non_zeroes)}, should be: {dim}"
            )
...
def trace(q_1: Qs) -> Qs:
    ...
    if q_1.rows != q_1.columns:
        raise ValueError(
            f"Oops, not a square quaternion series: {q_1.rows}/{q_1.columns}"
        )
